...
//!
//! Lint crate (theorist M7) that enforces roadmap §6 theorist Minor 1 —
...
//! build on it.
//!
//! ## DIP-N4 N4.10 — Single-point-of-failure (SPOF) analysis
//!
//! This crate has accreted three workspace-wide structural invariants
//! across the Track A.1 → Track H DIP cycles, all surfaced through
//! integration tests under `tests/`:
//!
//! 1. **Trait default-body fingerprint** (DIP-N1 Track A.1 / theorist
//!    M7) — `TraitDefaultFingerprint` types here in `src/lib.rs` plus
//!    the proc-macro tests that exercise them. Anchors roadmap §6
//!    Minor 1 ("a semantic change to a default-impl body is a breaking
//!    change").
//! 2. **`ActionCompute` workspace coverage** (DIP-N1 E14.L1 lint trip)
...
//! 3. **Track H §14.7 forward-looking event 0-emission** (DIP-N3 Track
//!    H.3 Layer (d)) — `tests/forward_looking_event_zero_emission.rs`
//!    scans Runtime production source for any reference to
//!    `ReplicaIdAllocation` / `AuditReceiptKeyPolicy` outside the two
//!    allowed files. Mechanical proof that the v0.12 sealed cut emits
//!    these define-only events from zero production sites.
//!
//! ### SPOF risk
//!
...
//!
//! ### Mitigation v0.12 — count-gate sentinel
//!
//! The workspace's default test count gate (`cargo test --workspace`,
//! 540 at the DIP-N4 N4.9 close) acts as the implicit meta-check:
//! disabling or deleting any of the three invariant tests trips the
//! workflow-3 default test count delta — peer reviewers flag the count
//! decrease, and the architect / auditor / cryptographer cycle catches
//! the regression before merge. **Test count gate IS the mechanical
//! sentinel for the SPOF**.
//!
...
//! - Per-feature `cargo deny` (DIP-N3 H.2) — supply-chain regression
//!   would be caught at deny time, independent of the test layer.
//!
//! ### Migration-path note (DIP-N4 N4.10)
//!
...
//!
//! **v0.13+ candidate #13.1** (cryptographer + auditor + architect
//! convergent at DIP-N4 N4.10): when invariant N+1 is added, upgrade
//! the SPOF mitigation from this rustdoc + count-gate sentinel to a
//! mechanical meta-test (lib-move + thin wrapper + fn pointer
//! collection). Counter-balanced against the YAGNI cost analysis at
//! DIP-N4 N4.10 (current count-gate sentinel suffices for v0.12 with
//! 3 invariants).
