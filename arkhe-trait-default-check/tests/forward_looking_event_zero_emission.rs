//! Track H.3 (DIP-N3 v0.12 sealing-cycle) — 3-layer 0-emission
//! defense Layer (d) **mechanical proof**.
//!
...
//!
//! # Defense layers (cryptographer + auditor cycle plan agreement)
//!
...
//!   default builds skip compiling the types entirely. Mechanical
//!   proof at H.2 commit (`ea75ef8`): default tests 521 → 515 (-6
//!   cfg-gated).
...
//! emission** at v0.12, with mechanical evidence at all three layers.
//!
//! # Failure mode
//!
...
//! structural-invariant `syn` scaffolding. D-USER-4 "do one thing
//! well" reads workspace-wide structural invariant scan as a single
//! dimension covering trait-default-body fingerprint (E14.L1 / R4-J),
//! Action-compute coverage (E14.L1 lint trip), and now Track H
//! 0-emission Layer (d). The leader retroactively adopted this
//! framing at A.1 cycle plan (Option γ).
...
fn forward_looking_events_have_zero_production_emit_sites() {
    ...
    assert!(
        violations.is_empty(),
        "Track H.3 Layer (d) — forward-looking event types referenced outside the \
