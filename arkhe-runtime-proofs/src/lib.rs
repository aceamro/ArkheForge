...
//!
//! Implementation-level Kani harness suite for the L1 Action submit /
//! dispatch / replay path. The crate is **excluded from the root
//! ArkheForge workspace** — `cargo build` and `cargo test --workspace`
//! from the workspace root do not see this crate. Kani harnesses run
...
//!    = 8 bounded cases (binary typestate domain).
...
/// cases (binary typestate domain).
///
/// **Anchor**: E-series axioms E6 + E7 (TYPE-PROVEN at Rust + abstract
/// MC at Kani).
#[cfg(kani)]
#[kani::proof]
#[kani::unwind(8)]
fn kani_authorize_property() {
    ...
    // Symbolic shell brands: 0..4 (N=4 bounded domain)
...
///
/// **Anchor**: E-series axiom E14 (MACHINE-CHECKED build-time L1 +
/// runtime L2). Implementation-level proof anchor for A1 bit-identical
/// replay (E14 ⇒ A1).
#[cfg(kani)]
#[kani::proof]
#[kani::unwind(8)]
...
///
/// **Bounded MC**: 3-event window (k=3), each event is a u8 byte
/// (256-element symbolic domain per slot). Total symbolic state =
/// 256^3 = 16M cases. SMT unwind = 8.
///
...
mod hybrid_and_mode_model {
    ...
    //! `verify_chain`. Forward-secure default for new WALs.
    //!
