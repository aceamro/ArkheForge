...
//! `Vec<Op>`. The L2 service layer calls [`ActionContext::drain_ops`]
//! after compute returns, wraps each `Op` in a fresh
//! `Effect<'i, Unverified>`, and submits through the L0 authorize →
//! dispatch path. This architecture is driven by the L0 API — `Effect`
//! wraps a single `Op` with `pub(crate)` fields, so it cannot be threaded
//! through a long-running Runtime accumulator directly.
//!
//! The `'i` brand is the instance lifetime; a future release binds it to
//! the L2 `Kernel::submit` call-site so the Runtime context cannot escape
//! the WAL transaction it was opened inside.
...
///
/// v0.11 ships an in-memory implementation in `arkhe-forge-platform::dedup`;
/// the production path swaps in a PG-UNIQUE-INDEX-backed impl (spec §14.8).
/// The L0 kernel may later layer a WAL-scan fallback underneath the same trait.
...
///
/// The `'i` lifetime will become the L0 `Effect<'i, Authorized>` brand
/// once a future release wires the bridge — currently it is a phantom to
/// reserve the position on the public API.
...
impl<'i> ActionContext<'i> {
    ...
    /// [`EventRecord`] buffer (consumed by the current skeleton pipeline)
    /// **and** pushed onto the L0 `Op::EmitEvent` accumulator (drained via
    /// [`ActionContext::drain_ops`] by the L2 service layer).
    ...
    /// balance the memory budget (L0 quota discipline). A future release
    /// reads the value from the kernel state; current callers pass the
    /// original size explicitly.
    ...
    /// otherwise returns `None` for forward-compat (spec §3.3 NR6-3).
    ///
    /// The v0.11 production path uses PG UNIQUE INDEX (spec §14.8); the
    /// L0 kernel may later layer a WAL-scan fallback beneath the same
    /// interface.
    #[inline]
    #[must_use]
