...
/// `RoomMarker` TypeCode reservation (SHAPE-only at v0.12 per
/// D-USER-5 (iii); spec §14.1.1 Room primitive — SHAPE-only declaration).
///
/// **Sealed-completeness mutual lock** (cryptographer Q7 anchor): this
/// doc-only constant pairs with the spec-text declaration of
/// `RoomMarker: EntityKind` in `runtime-book/src/en/architecture/14-open-questions.md`
/// §14.1.1. Two sources together (spec text + code constant) prevent
/// v0.13 implementers from accidentally re-allocating `0x0001_5001`
/// for a different `EntityKind` impl — sealing-cut foreclosure
/// preserved without committing the implementation surface.
///
/// **v0.12 first-cut scope** (SHAPE-only, Track C.2):
///
/// - `EntityKind` trait + per-marker `impl` blocks remain spec-text-
///   only at v0.12 (deferred to v0.13 alongside the §14.6 ArkheUri
///   implementation surface).
/// - **0 Component / 0 Action / 0 VerbCode** allocations under
///   `RoomMarker` at v0.12. Room-specific verbs land via the
///   M-verbrange shell BLAKE3 sub-allocation pattern (§14.4) at v0.13,
///   NOT as canonical verbs in `CORE_VERB_CANONICAL`.
/// - Future v0.13+ cap-token family `arkhe:room/{join, post, leave, ...}`
///   is an independent dimension — this constant reserves only the
...
pub mod core_event {
    ...
    /// `SignatureClassPolicy` — §14.7 FG5 chain-anchored policy / E13.
    ...
    /// `PerRegionErasureProgress` — §14.9.1 §§13 GF4 2PC.
    ...
    /// registration receipt. Track B.6 (DIP-N1 v0.12 sealing cycle).
    pub const HOOK_MODULE_REGISTER: u32 = 0x0003_0F0B;
    /// `ObserverQuarantine` — §14.5.2 / E15 Observer host v2 chain-anchored
    /// trap-quarantine receipt. Track A.2.4 (DIP-N2 v0.12 sealing cycle).
    /// Emitted by the runtime supervisor when an observer wasm execution
    /// trips a sandbox-boundary failure — the host catches the trap +
    /// generates this event so the chain anchors observer compromise
    /// without observer wasm authorship of the receipt (cryptographer
    ...
    /// receipt. Track H.1 (DIP-N3 v0.12 sealing cycle, define-only).
    ///
    /// **0-emission at v0.12** — runtime ships the type definition and
    /// reserves the wire slot, but no production code path ever calls
    /// `emit_event::<ReplicaIdAllocation>(..)`. Emission activates at
    /// v0.99+ once the federation prerequisites (archive-hardening,
    /// SignedArkheUri, identity federation layer per §15.5 / §14.7)
    /// are met. The H.2 sub-step wraps the type definition behind the
    /// `federation-archive-hardened` Cargo feature so default builds
    /// do not even compile the type.
    pub const REPLICA_ID_ALLOCATION: u32 = 0x0003_0F0D;

    /// `AuditReceiptKeyPolicy` — §14.7 audit-receipt key inventory /
    /// rotation manifest. Track H.1 (DIP-N3 v0.12 sealing cycle,
    /// define-only).
    ///
    /// **0-emission at v0.12** — production code path never emits;
    /// activation requires the operator-side carry-over (g) "audit-
    /// receipt key identity declared in `release-keys.md §1`
    /// inventory" (cryptographer Track H gate). H.2 wraps the type
    /// behind the `audit-receipt-key-identified` Cargo feature.
...
mod tests {
    ...
    #[test]
    fn room_marker_reserved_value_is_pinned_to_v0_12_slot() {
        assert_eq!(ROOM_MARKER_RESERVED, 0x0001_5001);
    }

    /// Sealed-completeness mutual lock (cryptographer Q7 anchor): the
    ...
    /// SHAPE-only freeze (D-USER-5 (iii)): no other Core Entity
    /// constant can collide with the reserved slot. v0.12 Core Entity
    /// allocations are limited to existing UserMarker / ActorMarker /
    /// SpaceMarker / EntryMarker / ActivityMarker (per §14.6) plus
    /// the SHAPE-only `RoomMarker` reservation.
    #[test]
    ...
    // ----- Track H.1 (DIP-N3) — forward-looking event TypeCode reservations -----

    /// `ReplicaIdAllocation` lands at the next free Core Event slot
    /// after `ObserverQuarantine 0x0003_0F0C` (Track A.2.4 / DIP-N2).
    /// v0.13+ implementers must not collide.
    #[test]
    ...
    /// (no double-allocation within Track H.1).
    #[test]
