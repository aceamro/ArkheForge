...
- **`pqc-hybrid`** — post-quantum MlDsa65 + hybrid signing (opt-in preview).
- **`unstable`** — pre-freeze API opt-in.
...
- `crypto` — HSM-generated DEK + envelope encryption, tombstone semantics,
  19-byte AEAD AAD.
- `hf2_kms` — HF2 auto-promote trust model: multi-channel health check (DoH /
...
```toml
[dependencies]
arkhe-forge-platform = { version = "0.11", features = ["tier-1-kms"] }
```

```rust
use arkhe_forge_platform::PLATFORM_SEMVER;
assert_eq!(PLATFORM_SEMVER, (0, 11, 0));
```
