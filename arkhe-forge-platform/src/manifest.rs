...
pub struct RuntimeSection {
    ...
    /// Runtime semver the manifest author tested against (e.g. `"0.11"`).
...
pub struct AuditSection {
    ...
    /// `"after_60min"` (HF2 auto-promote after 60 minutes of health-check
...
impl ManifestLoader {
    ...
    pub fn validate(m: &ManifestSnapshot) -> Result<(), ManifestError> {
        ...
        // software-kek refused past runtime 0.15 (GF1).
    ...
    /// emitted bytes stay stable across builds in the v0.11 frozen window.
    ///
...
/// pre-release / build-metadata suffixes (spec §14.7 NR6-6 — Runtime does
...
"#;
    ...
    fn print_tier0_dev_digest_for_pin() {
        // One-shot helper to discover the v0.11 sentinel bytes; left in the
        // suite so future maintainers can re-run it after a deliberate
        // schema bump (spec §5.6) and copy the new bytes into
        // `digest_invariant::TIER0_DEV_DIGEST_V0_11`.
        let (_, d) = ManifestLoader::load(TIER0_DEV_TOML.as_bytes()).unwrap();
        eprintln!(
            "tier0_dev v0.11 digest = {:?}",
    ...
    fn software_kek_past_0_15_refused() {
        let bad = TIER0_DEV_TOML
            .replace("runtime_current = \"0.11\"", "runtime_current = \"0.16\"")
    ...
    fn runtime_max_less_than_current_rejected() {
        let bad = TIER0_DEV_TOML.replace("runtime_max = \"0.15\"", "runtime_max = \"0.10\"");
        let err = ManifestLoader::load(bad.as_bytes()).unwrap_err();
    ...
    fn parse_version_accepts_two_or_three_components() {
        assert_eq!(parse_version("0.11"), Some((0, 11, 0)));
        assert_eq!(parse_version("0.11.3"), Some((0, 11, 3)));
        assert_eq!(parse_version(""), None);
        assert_eq!(parse_version("0.11.3.4"), None); // 3-part splitn leaves "3.4"
...
/// output. `Cargo.lock` pins the toml minor version, so within the v0.11
/// frozen window the bytes below are stable. A future toml major bump
...
mod digest_invariant {
    ...
    /// toml::to_string(TIER0_DEV_TOML_snapshot))` under the v0.11
    /// `Cargo.lock`-pinned `toml` crate version. Captured 2026-04-25.
    const TIER0_DEV_DIGEST_V0_11: [u8; 32] = [
        0x69, 0x46, 0x83, 0x7d, 0x41, 0x30, 0x74, 0xb0, 0x8a, 0x34, 0x5b, 0x5a, 0xb9, 0x14, 0x75,
        0xe9, 0xb0, 0xf3, 0xe3, 0x22, 0x19, 0x7a, 0xb7, 0x0b, 0x9a, 0xb0, 0x46, 0x11, 0x17, 0x80,
        0x93, 0x84,
    ];
    ...
    fn tier0_dev_digest_matches_v0_11_sentinel() {
        ...
        assert_eq!(
            ...
            "manifest canonical_digest drifted from the v0.11 sentinel — check toml crate \
