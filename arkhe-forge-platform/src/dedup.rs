//! L2 idempotency-key dedup service (spec §14.8 / §3.3 NR6-3).
//!
//! v0.11 primary path: PG UNIQUE INDEX on the `idempotency_keys` projection
//! table. This module ships the trait surface plus an in-memory
//! implementation suitable for Tier-0 dev and unit tests. The PG-backed
//! implementation and a potential L0 kernel WAL-scan fallback (FG6) plug in
//! under the same [`IdempotencyIndex`] contract.
