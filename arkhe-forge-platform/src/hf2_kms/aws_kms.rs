...
//! L2 coordinators (spec §14.11.2 HF2 backend boundary).
//!
...
fn map_sdk_error<E: std::fmt::Display>(e: E) -> KmsError {
    ...
    // — the v0.11 surface collapses them into the coarse [`KmsError`]
...
impl KmsBackend for AwsKmsBackend {
    ...
    fn delete_key(&self, kek_ref: &KekRef) -> Result<KeyDeletionAttestation, KmsError> {
        self.block_on(async {
            ...
            // via a separate attestation service (spec §14.11.3 FG3).
    ...
    fn rotate_kek(&self, old: &KekRef, new: &KekRef) -> Result<(), KmsError> {
        // AWS exposes `UpdateAlias` + `EnableKey` / `DisableKey`. v0.11 pins
        // the contract surface only — operators execute the alias swap via
        // `aws kms update-alias` in the rotation runbook. The runtime marks
        // the logical rotation attempted by successfully describing both KEKs.
        self.block_on(async {
            for k in [old, new] {
                self.client
                    .describe_key()
