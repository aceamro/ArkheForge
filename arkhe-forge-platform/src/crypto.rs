...
/// (v0.99+) populate a per-replica id via [`Dek::with_config`].
///
...
///
/// `replica_id = 0` is the single-writer default (current v0.11
/// deployments). Federation builds (v0.99+) supply a per-replica id
/// via [`DekConfig::replica_id`] so two regions sharing the same DEK
/// material cannot collide their counters. The value is captured at
/// `Dek` construction and is immutable for the lifetime of the key
/// (L0 A11 pure compute); changing it later would surface in the
/// WAL as different ciphertext bytes and therefore must ship behind
/// a manifest-anchored policy event in the federation DIP.
#[cfg_attr(not(feature = "tier-2-multi-kms"), allow(dead_code))]
#[inline]
