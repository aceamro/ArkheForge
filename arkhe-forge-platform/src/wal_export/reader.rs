//! `WalStreamReader` — production reader-side surface (DIP-N4 N4.8).
//!
...
//! payload borrows. The complement to the writer-side surface that
//! Track F sealed at v0.12.
//!
...
//!
//! # Fail-secure framing posture (DIP-N4 N4.8)
//!
//! Five reject paths cover the framing surface:
//!
//! 1. **Unknown magic** at stream open → [`WalExportError::UnsupportedStreamVersion`]
//!    (DIP-N4 N4.7 dispatch). Stack-only 8-byte read, no heap alloc
//!    pre-rejection (cryptographer Q5 fail-fast).
...
//!
//! # Bridge to F.4 round-trip tests
//!
//! F.4 `parse_stream` test-only helper inlined a similar parser. This
//! module hosts the production-grade equivalent that F.4 integration
//! tests can opt into post-N4.8.
...
pub struct StreamingWalReader<R: Read> {
    ...
    cumulative_pos: u64,
    /// The recognised stream magic (always `V1` at v0.12; future
...
impl<R: Read> StreamingWalReader<R> {
    ...
    /// header magic.
    ///
    /// **Fail-fast posture** (cryptographer Q5 carry-forward): the
    /// 8-byte header is read into a stack array; an unrecognised
    /// magic yields [`WalExportError::UnsupportedStreamVersion`]
    /// with no heap allocation pre-rejection. A truncated header
    /// (fewer than 8 bytes) yields
...
mod tests {
    ...
    fn open_v1_unknown_magic_rejected_with_unsupported_version() {
        let unknown: &[u8] = b"ARKHEXP9"; // future version not yet supported
    ...
    /// defence — cryptographer N4.7 carry-forward).
    #[test]
