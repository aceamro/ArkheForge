//! Round-trip + replay determinism integration tests (Track F.4).
//!
...
//! ```
//!
//! All six F.4 cross-review carry-forwards covered:
//!
...
mod tests {
    ...
    ///
    /// **Test-only**. Production reader-side surface is v0.13+ scope.
    ...
    // -------------------------------------------------------------------
    // Carry-forwards #1 + #4: round-trip + replay determinism + chain integrity
    // -------------------------------------------------------------------

    /// **CF #1 + #4**: stream a real Kernel-driven WAL through the sink,
    /// parse it back, and confirm `Wal::verify_chain` still passes.
    ...
    /// **CF #1**: reconstructed records equal originals byte-for-byte
    /// (postcard re-encoding produces identical bytes via Serialize).
    #[test]
    ...
    // -------------------------------------------------------------------
    // CF #2: stream header / invalid prefix reject scenarios
    // -------------------------------------------------------------------

    /// **CF #2a**: empty stream → `Truncated` (header section missing).
    #[test]
    ...
    /// **CF #2b**: stream without header magic → `HeaderMissing`.
    #[test]
    ...
    /// **CF #2c**: stream with header but truncated mid-length-prefix →
    /// `Truncated`.
    #[test]
    ...
    /// **CF #2d**: zero-length prefix → `LengthZero`.
    #[test]
    ...
    /// **CF #2e**: oversized length prefix → `LengthExceedsMax`.
    #[test]
    ...
    /// **CF #2f**: stream truncated mid-payload → `Truncated`.
    #[test]
    ...
    // -------------------------------------------------------------------
    // CF #3: tamper detection
    // -------------------------------------------------------------------

    /// **CF #3**: tampering a record's chain hash bytes mid-stream
    /// breaks `verify_chain`. The framing layer (parse_stream) accepts
    /// the bytes as well-formed, but downstream chain verification
    /// catches the integrity break.
    #[test]
    fn tampered_record_section_breaks_verify_chain() {
        ...
        //
        // **Note (cryptographer F.4 doc anchor)**: perturb_offset = 32
        // assumes byte 32 within record 0 lands in the chain-hashed
        // payload region. If a future WalRecord schema migration
        // (DIP-N3 v0.13+ candidate #8 — schema migration coordination)
        // shifts the layout, this offset may need re-tuning to stay
        // inside the hashed region.
    ...
    // -------------------------------------------------------------------
    // CF #5: replay-attack (record duplication) detection
    // -------------------------------------------------------------------

    /// **CF #5 (cryptographer F.3 신규)**: producer-side rejection —
    /// `BufferedWalSink::append_record` rejects a duplicate `seq` with
    /// `AppendOnlyViolation`.
    #[test]
    ...
    /// **CF #5 consumer-side**: a duplicated record in the parsed
    /// stream (constructed manually here) breaks `verify_chain` — the
    ...
    // -------------------------------------------------------------------
    // CF #6: mid-stream magic insertion detection
    // -------------------------------------------------------------------

    /// **CF #6 (cryptographer F.3 신규)**: inserting
    /// [`STREAM_HEADER_MAGIC`] bytes mid-stream produces a bogus length
    /// prefix that exceeds `MAX_RECORD_BYTES`. The 8-byte magic
    /// `ARKHEXP1` decoded as u64 BE is `0x4152_4B48_4558_5031` ≈ 4.7
    /// quintillion — far above `MAX_RECORD_BYTES = 1 << 24`. Parser
    /// rejects with `LengthExceedsMax`, defending against split-stream
    /// concatenation attacks.
    ///
    /// **Defence-in-depth split** (cryptographer F.4 doc anchor):
    ...
    ///   hashes → covered by
    ///   [`tampered_record_section_breaks_verify_chain`] (CF #3).
    ///
    ...
    ///
    /// **Combined determinism coverage matrix** (cryptographer F.4 doc
    /// anchor): F.3
    /// [`stream_framing_golden_vector`](super::wire_stability) pins a
    /// fixed input → fixed hex output (catches compiler / library /
    ...
    /// DIP-N4 N4.9 — bridge test verifying `BufferedWalSink::extract_seq`
    /// tracks the L0 `WalRecord` schema's leading `seq: u64` field
    /// (DO NOT TOUCH #8 sentinel).
    ///
    /// Path:
    ...
    ///
    /// Cryptographer F.2 carry-forward (cand #7): WalRecord schema
    /// migration coordination check. Pre-N4.9, the `extract_seq`
    /// path's coupling to L0 schema was implicit — Runtime tests
    /// exercised the full pipeline but did not isolate the schema-
    /// coupling invariant. N4.9 lifts the invariant to an explicit
    /// type-level + test-level pin.
    #[test]
