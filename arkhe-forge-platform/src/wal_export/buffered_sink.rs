//! `BufferedWalSink<W>` — concrete WAL streaming sink (Track F.2).
//!
...
//!
//! Module-level invariant (architect anchor): **`BufferedWalSink` is
//! the sole `pub` write path between the L1+L2 Runtime and operator-
//! managed durable storage**. The sink:
//!
//! - exposes neither `Seek` nor any inherent positioning method —
//!   compile-time `static_assertions::assert_not_impl_any!` enforces
//!   the absence of `std::io::Seek` (cryptographer F.2 carry-forward
//!   #2). Adding a `Seek` impl in a future PR fails to compile.
...
//! `seq == previous + 1`.
//!
//! # u64 seq wraparound — fail-secure reject (F.2 carry-forward #4)
//!
//! `previous.checked_add(1)` returns `None` at `u64::MAX` →
//! [`WalExportError::AppendOnlyViolation`] explicit reject. Practical
//! impact ≈ zero (1M ops/sec ≈ 584942 years to overflow), but the
//! check closes the structural gap rather than relying on the L0
...
//!   fail-secure ceiling, F.1 anchor).
...
// production path now goes through `StreamMagic::V1.bytes()` per
// DIP-N4 N4.7). Cfg-gating keeps the lib build warning-free without
// hiding the import behind `#[allow]`.
#[cfg(test)]
...
// `std::io::Seek` (cryptographer F.2 carry-forward #2). Adding a
// `Seek` impl in a future change fails compilation here, surfacing
// the trait-level append-only invariant violation BEFORE merge.
//
// Coverage shape (DIP-N4 N4.6 — std-only zero-new-dep posture per
// cryptographer + auditor convergent decision):
//
...
//    shapes (cryptographer F.2 belt-and-suspenders).
// 3. `&mut Vec<u8>` (DIP-N4 N4.6) — the borrow-form of in-memory
//    buffer used by callers that own the underlying allocation
//    elsewhere; closes the auto-trait-leakage path where a borrow's
//    `Seek` could differ from the owned type's.
// 4. `std::io::BufWriter<Vec<u8>>` (DIP-N4 N4.6) — the buffered-
//    writer wrapper commonly used to amortise small writes; the
//    wrapper does NOT implement `Seek` itself but the assertion pins
//    the invariant against any future stdlib auto-impl change.
// 5. `std::io::Cursor<Vec<u8>>` (DIP-N4 N4.6) — the cursor wrapper
//    commonly used in test fixtures; `Cursor<Vec<u8>>` implements
//    `Seek` (unlike bare `Vec<u8>`), so a careless `impl Seek for
//    BufferedWalSink<W>` delegating to inner would compile here,
//    making this the strictest of the five assertions.
//
// Async wrappers (`tokio::fs::File`, `async_std::fs::File`,
// `tokio::io::AsyncSeek`) intentionally NOT covered: v0.12 Runtime
// is sync-only, and adding tokio/async-std as dev-deps purely for
// compile-time guards = bloat. Future async expansion DIP would
// add the corresponding assertions alongside the async surface.
...
impl<W: Write> BufferedWalSink<W> {
    ...
    ///
    /// **L0 schema coupling — DO NOT TOUCH #8 sentinel** (DIP-N4 N4.9
    /// anchor): this function depends on `WalRecord` declaring `seq:
    /// u64` as its FIRST field. postcard encodes structs field-by-
    /// field in declaration order, so the first
    /// `take_from_bytes::<u64>` of the record bytes yields seq
    /// without deserializing subsequent fields. If a future schema
    /// migration reorders `WalRecord` fields, the bridging test
    /// `walrecord_leading_seq_invariant_bridge` (in `round_trip_tests`)
    /// fails before any production wire damage occurs. The L0 invariant
    /// is `arkhe_kernel`'s "DO NOT TOUCH #8" anchor — Runtime layer
    /// holds this `pub(super)` accessor so the bridge test can verify
    /// the coupling at every cycle without exposing the function to
    ...
    ///
    /// DIP-N4 N4.7 — uses [`StreamMagic::V1`] dispatch for forward-
    /// compat: a future writer wanting `V2` would call
    /// [`StreamMagic::V2.bytes()`] here without touching the rest of
    /// the buffered-sink machinery. The byte pattern is identical to
    /// [`STREAM_HEADER_MAGIC`] (wire-stability invariant pinned by
    /// the F.3 golden vector).
    ...
    /// [`super::wire_stability`] module (Track F.3) to inspect the
    /// post-flush byte stream. Production code path should never see
    /// this method.
    #[cfg(test)]
...
impl<W: Write> WalRecordSink for BufferedWalSink<W> {
    fn append_record(&mut self, record_bytes: &[u8]) -> Result<(), WalExportError> {
        ...
        if let Some(prev) = self.last_seq {
            ...
            // variant (DIP-N4 N4.5 — distinct from append-only
            // violation since this signals an intrinsic limit, not a
        ...
        // accepted as the first record's anchor. This preserves F.4
...
mod tests {
    ...
    /// payload contents are opaque to the sink. F.4 covers the
    ...
    /// Out-of-sequence seq trips `AppendOnlyViolation`. DIP-N4 N4.5
    /// — verifies the new `previous_seq: Some(prev)` forensic field.
    #[test]
    ...
    /// (replay attack defence). DIP-N4 N4.5 — verifies forensic
    /// `previous_seq` reflects the current high-water mark.
    #[test]
    ...
    /// capacity is exceeded → `BufferOverflow`. DIP-N4 N4.5 — verifies
    /// the new `current_buffer` forensic field reports the bytes
    /// already in the buffer at overflow time (the predicate
    /// `current_buffer + requested > capacity` is what triggers).
    #[test]
    ...
    /// dedicated `SeqExhausted` variant (DIP-N4 N4.5 — distinct from
    ...
    /// (allocating 16 MiB+1 in tests is wasteful); F.4 covers
    /// real-world boundary in integration.
    #[test]
