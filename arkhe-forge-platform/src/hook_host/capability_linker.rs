//! Capability-bounded wasmtime [`Linker`] template (Track B.3.a).
//!
...
//! 2. **Link-time deny-by-default** — [`CapabilityLinker::v0_12_first_cut`]
...
//!
//! - `arkhe:hook/state.read` — bound at Track B.3.a (call-stub: traps
//!   until Track B.5 wires the actual scratchpad).
//! - `arkhe:hook/state.write` — bound at Track B.3.b (call-stub).
//! - `arkhe:hook/emit.extra_bytes` — bound at Track B.3.b (call-stub).
//! - `arkhe:hook/fuel.consumed` — bound at Track B.4 (**fully working**;
//!   returns `initial_fuel - Caller::get_fuel()`, no scratchpad needed).
//!
...
//! (cryptographer Q1 / spec §5.3 confused-deputy).
...
/// these. v0.12 first cut admits only the `arkhe:hook/*` namespace —
/// everything else (including all WASI namespaces) is out of scope.
///
...
/// DRY consolidation, DIP-N6 Phase 2). Hook + observer share the
...
///
/// Cryptographer C7 v0.12. Maintenance: kept in sync with the WASI
/// Preview 2 namespace inventory at the shared site (one update point,
/// both hosts inherit).
...
///
/// Future Track B sub-steps extend this struct (state map, extra-bytes
/// handle) — the shape is `#[non_exhaustive]` so additive growth does
/// not break out-of-tree consumers.
///
/// Field genealogy:
/// - Track B.3.a: `capabilities`.
/// - Track B.4: `initial_fuel` (fuel.consumed host-fn computation).
/// - Track B.5.b: `scratchpad` (state.read/state.write actual key→value
///   storage) + `extra` (emit.extra_bytes builder ownership thread).
#[non_exhaustive]
...
pub struct HookStoreData {
    ...
    /// consumption (`initial - remaining`). v0.12: Track B.4.
    ...
    /// Track B.5.b: populated/queried by the `arkhe:hook/state.{read,
    /// write}` host-fns. Lives for the duration of one `invoke()`
    /// call — fresh map per invocation, contents discarded at end.
    /// Persistence across invocations is a v0.13+ DIP candidate
    /// (currently the host's threat model assumes per-invocation
    /// statelessness for the hook author's view).
...
impl HookStoreData {
    ...
    /// future operator-supplied initial state (v0.13+). v0.12 host
    /// production callers always pass an empty iterator (per-invocation
    /// statelessness).
...
/// [`super::wasmtime_host::WasmtimeHookHost`] (which itself is per-shell
/// in the eventual Track B.6+ deployment) and reused across every
/// invocation. Per-invocation `Store<HookStoreData>` instances bring the
/// capability set; the linker holds only the host-fn bindings.
///
...
// M2.5 cycle (DIP-N6 Phase 2): SealedHostImport safeguard. The hook
// `CapabilityLinker` impls both the private `Sealed` marker (only
...
// preserving the **CR-1 HostImports compile-time invariant**: the
// wasmtime host-fn import surface (4-set hook host-fns) is structurally
// bounded at the host-defining crate boundary.
...
impl CapabilityLinker {
    /// Build the v0.12 first-cut linker. Binds four host-fns:
    ///
    /// | Import path                  | CapToken          | Sig                       | Behaviour                                        |
    /// |------------------------------|-------------------|---------------------------|--------------------------------------------------|
    /// | `arkhe:hook/state.read`      | `StateRead`       | `(i32, i32) -> i32`       | call-stub: traps with Track B.5 placeholder      |
    /// | `arkhe:hook/state.write`     | `StateWrite`      | `(i32, i32, i32, i32)`    | call-stub: traps with Track B.5 placeholder      |
    /// | `arkhe:hook/emit.extra_bytes`| `EmitExtraBytes`  | `(i32, i32)`              | call-stub: traps with Track B.5 placeholder      |
    /// | `arkhe:hook/fuel.consumed`   | `FuelConsumed`    | `() -> i64`               | **fully working** at B.4: returns `initial - remaining` |
    ///
    /// Each body inspects the per-invocation
    /// [`HookStoreData::capabilities`] set — if the matching
    /// [`CapToken`] is absent, trap with `CapabilityDenied`. The first
    /// three host-fns trap with a "Track B.5" placeholder when the
    /// capability IS present (real scratchpad / extra-bytes thread
    /// lands at B.5). `fuel.consumed` is metric-only (no scratchpad or
    /// linear-memory access required) and returns the live consumed
    /// fuel value.
    pub fn v0_12_first_cut(engine: &Engine) -> Result<Self, HookHostError> {
        let mut linker = Linker::<HookStoreData>::new(engine);
        // arkhe:hook/state.read — read scratchpad value by key bytes
        // into a wasm-side output buffer. Track B.5.b real impl.
        //
        ...
        // Track B.5.b real impl. Bounds-check both (key) + (val)
        // ranges; insert/replace existing entry. No return value (void).
        linker
            .func_wrap(
                "arkhe:hook/state",
                "write",
        ...
        // submission's extra-bytes builder. Track B.5.b real impl.
        // Bounds-check (ptr, len), read bytes, append to the
        // `HookStoreData::extra` builder (must be Some — caller's
        // responsibility to seed via `with_extra`). If `extra` is `None`
        // a host-fn programmer error is reported as a trap rather than
        // silent drop; prevents accidentally-discarded extra bytes.
        linker
            .func_wrap(
                "arkhe:hook/emit",
                "extra_bytes",
        ...
        // Unlike the other three host-fns, this is fully implemented at
        // B.4: no scratchpad / linear-memory access needed, just a
        // wasmtime fuel API call. Returns `i64` (wasm has no unsigned
        // types in core wasm; the value is non-negative semantically).
        linker
            .func_wrap(
                "arkhe:hook/fuel",
                "consumed",
    ...
    /// instantiation. Track B.3.a leaves the actual instantiation
    /// wiring to a future sub-step (`WasmtimeHookHost::invoke` still
    /// returns the trap stub) — this accessor is the entry point for
    /// B.3.b+.
...
mod tests {
    ...
    fn engine() -> Engine {
        ...
        Engine::new(&cfg.to_config()).expect("v0.12 engine builds")
    }
    ...
    #[test]
    fn linker_v0_12_first_cut_constructs() {
        let _ = CapabilityLinker::v0_12_first_cut(&engine())
            .expect("v0.12 first-cut linker constructs");
    }
    ...
    /// Cross-host isolation reciprocal (cryptographer Q4 A.2.2 review):
    ...
    /// the wasm code itself running out. Track B.4 calibrates the
    /// production budget against the 10 ms wall-clock target; tests just
    /// need "plenty".
    const TEST_FUEL: u64 = 1_000_000;

    /// state.read cap-deny: missing `StateRead` capability → trap
    /// (Track B.5.b retains the cap-check; trap fires before any
    /// scratchpad lookup or memory access). Uses the new 4-arg
    ...
    fn state_read_traps_with_capability_denied_when_missing() {
        let engine = engine();
        let cap_linker = CapabilityLinker::v0_12_first_cut(&engine).unwrap();
        let bytes = wat_to_bytes(
            r#"(module
                (import "arkhe:hook/state" "read"
    ...
    /// the per-invocation scratchpad. Track B.5.b real impl.
    #[test]
    ...
    /// smaller than the value length. Track B.5.b real impl.
    #[test]
    ...
    fn debug_format_lists_prefixes() {
        let cl = CapabilityLinker::v0_12_first_cut(&engine()).unwrap();
        let s = format!("{cl:?}");
    ...
    // ---- B.3.b: C1 boundary tests + state.write + emit.extra_bytes ----

    /// C1 (cryptographer): `wasi:randomly-pure` shares the `wasi:random`
    ...
    #[test]
    fn c1_boundary_check_routes_lookalike_to_allow_list_message() {
        let bytes = wat_to_bytes(
            r#"(module
    ...
    /// C1: bare `wasi:random` (no `/` or `@`) at the namespace position
    /// hits the boundary check via `trailing.is_empty()` — denied.
    #[test]
    fn c1_boundary_check_admits_exact_prefix_match() {
        let bytes = wat_to_bytes(
            r#"(module
    ...
    /// C1: WIT `package@version/interface` form (`@` boundary) stays in
    /// the deny-list — denied with the specific-error reason.
    #[test]
    fn c1_boundary_check_admits_at_version_suffix() {
        let bytes = wat_to_bytes(
            r#"(module
    ...
    /// memory-write path. Track B.5.b real impl end-to-end.
    #[test]
    ...
    /// for downstream consumption. Track B.5.b real impl end-to-end.
    #[test]
    ...
    /// silently-discarded extra bytes. Track B.5.b real impl.
    #[test]
    ...
    // ---- B.4: fuel.consumed bind + initial_fuel + DENIED list sync ----
    ...
    /// Verifies the v0.12 first-cut Engine config's fuel-metering is
    /// actually wired through `Engine::new` → `Config::consume_fuel(true)`.
    #[test]
    ...
    // ---- B.5.a: memory bounds-check OOB tests ----
    ...
    /// arguments literal. Track B.5.b real-impl signature.
    ...
    /// scratchpad returns -1 (not found). Track B.5.b: previously this
    /// test asserted a B.5-stub trap; now real impl returns -1.
    #[test]
