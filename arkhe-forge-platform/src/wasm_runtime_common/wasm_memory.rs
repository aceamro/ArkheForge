...
//! and observer hosts (DIP-N1 B.5 firm contract).
//!
...
//!
//! 1. negative `len` → trap
//! 2. negative `ptr` → trap (BEFORE `len == 0` short-circuit, C16
...
/// at `ptr`, applying the cryptographer-pinned B.5 bounds-check
/// contract (DIP-N1 B.5.a / DIP-N2 A.2.2 shared). All `(ptr, len)`
/// host-fn arguments MUST pass through this helper before the host
/// touches wasm memory.
///
/// Generic over the `wasmtime::Store` data type `T` — both
/// `HookStoreData` (chain-affecting) and `ObserverStoreData` (chain-
...
///    Cryptographer C16 (DIP-N1): defense-in-depth ordering.
...
// feature), this helper is currently unused at A.2.2 — the observer
// dispatch that consumes it lands at A.2.3. The function compiles +
// type-checks under the observer-only feature so the contract is
// pinned ahead of A.2.3 dispatch wiring (cryptographer Q6 — drift
// avoidance via shared module).
#[allow(dead_code)]
...
/// `ptr`, applying the symmetric B.5 bounds-check contract for output
/// data flow (DIP-N1 B.5.b / DIP-N2 A.2.2 shared). Used by host-fns
/// that write back into wasm memory (e.g. `state.read` returning a
/// looked-up value).
///
...
//
// `dead_code` allowed: same rationale as `read_caller_memory` — used
// by hook host today, observer host activates the dispatch at A.2.3.
#[allow(dead_code)]
