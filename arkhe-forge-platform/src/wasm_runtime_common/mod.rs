...
//!
//! ## R4-X stratum classification (M2.6 cycle, DIP-N6 Phase 2)
//!
...
//!
//! **R4-X DAG invariant** (Layer A item 6): import direction is
...
//! invariant is enforced at CI by the M2.6 grep step (`grep -rE
//! "use\s+(crate::)?(hook_host|observer_host)"
//! arkhe-forge-platform/src/wasm_runtime_common/` must return 0 hits).
//!
//! ## Why shared
//!
//! Track A.2.2 (DIP-N2 v0.12 sealing cycle) extracts the bounds-check
//! helpers + parameterised import scan from `hook_host/capability_linker.rs`
//! into this module. Both wasmtime hosts need:
//!
//! - **`(ptr, len)` bounds-check** on every wasm-memory deref —
//!   cryptographer-anchored firm contract from DIP-N1 B.5.a (no FFI-
//!   shaped sandbox-escape primitive). Generic over the [`wasmtime::Store`]
//!   data type so the same body works for `HookStoreData` and
//!   `ObserverStoreData`.
...
mod tests {
    ...
    /// with all three E14.L2-Allow rules pinned. Verifies the M2.1
    ...
    /// M2.4 cycle (DIP-N6 Phase 2) — SealedCapToken safeguard
    /// compile-time witness. `assert_*_sealed::<C>()` requires
    /// `C: HookCapTokenSealed` (or `ObserverCapTokenSealed`) which is
    /// only satisfiable by types impl-ing the private `Sealed` marker.
    /// External crates cannot name `Sealed`, so cannot impl the trait
    /// — invariant raised to compile-time (theorist condition 2:
    /// `cargo expand` would show the sealed bound + `cargo doc`
    /// surfaces the sealed pattern).
    ...
    /// M2.5 cycle (DIP-N6 Phase 2) — SealedHostImport safeguard
    /// compile-time witness. `assert_sealed_host_import::<L>()`
    /// requires `L: SealedHostImport` which is only satisfiable by
    /// types impl-ing the private [`private_seal::Sealed`] marker.
    /// External crates cannot name `Sealed`, so cannot impl the trait
    /// — invariant raised to compile-time (CR-1 HostImports universe
    /// closed at host-defining crate boundary).
    fn assert_sealed_host_import<L: SealedHostImport>() {}

    /// Hook `CapabilityLinker` satisfies [`SealedHostImport`]. Compile-
    /// time witness for the M2.5 sealed-trait safeguard — verifies the
