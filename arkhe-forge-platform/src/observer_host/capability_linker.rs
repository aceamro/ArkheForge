...
//! 2. **Link-time deny-by-default** — [`ObserverCapabilityLinker::deny_by_default`]
...
//!   payload — no chain-mutation primitive.
...
//! ## Allow set
//!
//! - `arkhe:observer/pg.write` — concrete [`PgWriteCapability`] impl
//!   plus a [`MockPgWriteCapability`] test helper ship in this
//!   module. Additional capabilities (KMS / metric / etc.) can be
//!   added without breaking external matchers.
//!
...
/// with one of these. Admits only the `arkhe:observer/*` namespace —
...
/// consolidation). Observer + hook share the **identical 7-prefix
/// set** so the deny-list lives in one place.
///
...
/// — observers are read-only sinks (no `extra` builder, no
/// `scratchpad`). Observer-specific fields can be added additively as
...
pub struct ObserverStoreData {
    ...
    /// the `Store<ObserverStoreData>`. The field is kept for symmetry
    /// with `HookStoreData`; an observer-side `fuel.consumed` host-fn
    /// can be wired later without changing the struct shape.
...
/// (E15.b interface). Concrete impls (e.g. [`PgWriteCapability`])
/// carry the actual side-effect machinery — projection writes, KMS
/// calls, metric emits.
///
...
///   error the host records via the trap counter; downstream
///   integrations route this into the chain-anchored
...
pub trait ObserverCapability: std::fmt::Debug + Send + Sync {
    ...
    /// shared `read_caller_memory` helper (bounds-check pinned).
    ///
    ...
    /// structured error the host records via the trap counter for
    /// downstream `ObserverQuarantine` event routing.
...
/// boundary — chain-non-affecting → no chain-anchored Quarantine;
/// downstream integrations route these to a typed metric /
/// `runtime_doctor_journal` entry.
...
// SealedHostImport safeguard. The observer `ObserverCapabilityLinker`
// impls both the private `Sealed` marker (only reachable from
...
impl ObserverCapabilityLinker {
    /// Build the deny-by-default observer linker with the supplied
    /// capability set.
    ///
    ...
    /// registered" trap. This is the minimum-viable observer host
    /// deployment for operators who declare cap-tokens but haven't
    /// yet wired the concrete impl. The host's pre-scan + link-time
    /// layers still reject malformed modules.
    pub fn deny_by_default(
        ...
        // projection PostgreSQL table. Wasm signature
        ...
        // swallowed silently at the wasm boundary
        // (CapabilityExecutionError surfaces via metric/log alert,
        // NOT a chain-anchored Quarantine).
        //
        // Trap conditions (chain-anchored via Quarantine):
        ...
        // - Bounds-check failure on (ptr, len) memory access (firm
        //   contract — read_caller_memory).
        ...
        linker
            .func_wrap(
                ...
                move |mut caller: Caller<'_, ObserverStoreData>,
                    ...
                    // Downstream integrations route this to a typed
                    // metric / runtime_doctor_journal entry (NOT a
                    // chain-anchored Quarantine).
    ...
    /// Used by `WasmtimeObserverHost::invoke` (the trait impl from
    /// [`super::ObserverHost`]).
...
///
/// **Surface declaration**: this impl is the surface declaration only
/// — actual PostgreSQL connection wiring lives in operator shell
/// territory. The runtime ships the trait + a
/// [`MockPgWriteCapability`] in-memory test helper so the chain-non-
/// affecting integration test can run without a real PG dependency.
///
/// **Chain-non-affecting clause 2 enforcement**: the impl's `self`
/// field is chain-orthogonal. `PgWriteCapability` carries no field at
/// all (unit struct) — operator-side wiring may add a connection pool
/// / SQL template, both of which are PG-side concerns and never reach
/// the L0 chain.
#[derive(Debug, Default, Clone, Copy)]
pub struct PgWriteCapability;

impl PgWriteCapability {
    /// Construct a `PgWriteCapability` instance. Real PG wiring lives
    /// in operator shell territory — this constructor exists so
    /// operator code can declare the cap-token + register the impl
    /// without a PG dependency at compile time.
    pub fn new() -> Self {
        Self
    }
}
...
impl ObserverCapability for PgWriteCapability {
    ...
    fn execute(&self, _bytes: &[u8]) -> Result<(), CapabilityExecutionError> {
        // No PG wiring — this impl ALWAYS returns Ok(()) so operators
        // can declare the cap-token before the operator-side PG
        // integration lands. A real impl would:
...
/// downstream assertion. Used by chain-non-affecting integration
/// tests + future test-fixture builders.
///
/// **Why a separate type from `PgWriteCapability`**:
/// `MockPgWriteCapability` carries an `Arc<Mutex<Vec<Vec<u8>>>>`
/// field, which is *test-only* state. The production
/// [`PgWriteCapability`] stays a unit struct (zero state =
/// chain-orthogonal proof). Mixing the two via flag-toggled internal
/// state would muddy the structural-invariant review.
...
mod tests {
    ...
    fn pg_write_capability_execute_returns_ok() {
        // PgWriteCapability has no PG wiring — always Ok.
        let cap = PgWriteCapability::new();
    ...
    /// PgWriteCapability `self` fields are chain-orthogonal: zero
    /// fields (unit struct). The compile-time size check catches
    /// future field additions; PR reviewer must re-verify chain-
    /// orthogonality.
    #[test]
    fn pg_write_capability_is_zero_sized() {
        // Unit struct = zero-sized. Operator-side PG wiring may add a
        // connection-pool handle, but that's PG-side (outside the
        // chain) and never reaches L0.
