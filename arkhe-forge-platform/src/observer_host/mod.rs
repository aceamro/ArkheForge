//! Observer host v2 — capability-bounded WASM sandbox for L2 projection
//! observers (E15 — Observer Capability Confinement, v0.12 도입, MC).
//!
//! ## Spec anchor
//!
//! - **E15 Observer Capability Confinement** (v0.12 도입, MC) — Runtime axiom:
...
//! - Track A.2 (DIP-N2 v0.12 sealing cycle) — this module's introduction.
//!
...
//!    `ObserverCapability` impl carries its effect to a layer *outside*
//!    the chain (projection / metric / vault). Enforced by the trait
//!    signature shape — the impl receives no chain reference. Capability
//!    trait + concrete impls land in Track A.2.2 / A.2.3.
...
//! referenced only as conceptual anchor (`observer_host` exposes the same
//! `KernelEvent` observation pattern but inside a capability-bounded
//! WASM sandbox). Future v0.13+ work wires `observer_host` to L0's
//! `register_observer` call site; the binding glue is shell-territory or
//! a v0.13+ DIP and is intentionally out of scope for v0.12 sealing.
//!
//! ## v0.12 first-cut scope
//!
//! - [`ObserverHost`] trait + [`NoopObserverHost`] (Tier-0/1 default)
//! - [`ObserverContext`] / [`ObserverError`] / [`ObserverTrapClass`]
//! - [`ObserverCapToken`] enum (`#[non_exhaustive]`) — single v0.12 variant
//!   `PgWrite` (additional capabilities deferred to v0.13+ ecosystem-driven
//!   addition; see Track A.2 scope in `docs/runtime-sealing-plan.md`)
//! - `WasmtimeObserverHost` (feature `tier-2-observer-host-v2`) —
//!   wasmtime engine + per-host trap counter scaffold; module
//!   registration / capability linker / invoke pipeline land in
//!   subsequent Track A.2 sub-steps
...
///
/// `#[non_exhaustive]` — additive expansion is non-breaking. v0.12 ships
/// a single variant (`PgWrite`); KMS / metric / etc. capabilities are
/// deferred to v0.13+ ecosystem-driven addition (validated-repetition
/// directive — see Track A.2 scope in `docs/runtime-sealing-plan.md`).
///
...
// M2.4 cycle (DIP-N6 Phase 2): SealedCapToken safeguard. The observer
// `ObserverCapToken` enum implements both the private `Sealed` marker
// (only reachable from `arkhe-forge-platform`) and the public
// `wasm_runtime_common::ObserverCapToken` trait. External crates cannot
// satisfy the `Sealed` bound, so the observer-cap-token universe is
// closed at the host-defining crate boundary — preserving the E15.b
...
/// by the host. v0.12 first cut carries only the capability set;
/// observers are read-only sinks and do not mutate the submission
/// pipeline (cf. hook host's `ExtraBytesBuilder` thread).
///
...
// chain via the `ObserverQuarantine` event (Track A.2.4). Re-export
// keeps the platform-side API surface stable for callers that imported
// from `arkhe_forge_platform::observer_host::ObserverTrapClass`.
...
pub enum ObserverError {
    /// Observer invocation exceeded the fuel budget. v2-only — the
    ...
    /// Rejected at module-load (Track A.2.2 linker) or call-site
    /// (Track A.2.3 host-fn capability check).
...
/// # v1 alpha behaviour
///
/// Per spec §5.5 v1 alpha is [`NoopObserverHost`] — every call returns
/// `Ok(())` without invoking any wasm. Tier-2 deployments opt in to the
/// `WasmtimeObserverHost` (feature `tier-2-observer-host-v2`) for the
/// actual capability-bounded sandbox.
...
/// v1 alpha pass-through host — returns `Ok(())` without invoking any
/// observer logic. Tier-0 / Tier-1 deployments ship this; Tier-2
/// production opts in to `WasmtimeObserverHost`.
...
mod tests {
    ...
    #[test]
    fn observer_cap_token_v0_12_set_is_ordered_for_btreeset() {
        // PgWrite is currently the only variant; deterministic ordering
        // matters once additional v0.13+ variants land. Round-trip into
