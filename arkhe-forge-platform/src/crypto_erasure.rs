...
pub trait DekShredder: Send + Sync {
    ...
    /// Multi-region 2PC variant (spec §14.9.1 §§13 GF4). Real backends
...
/// Per-region shred progress entry (spec §14.9.1 §§13 GF4 two-phase commit).
///
...
pub struct ErasureCompletion {
    ...
    /// (spec §14.9.1 §§13 GF4 two-phase commit).
    pub regions: Vec<RegionProgress>,
}
...
impl ErasureCascadeObserver {
    ...
    /// as `PerRegionErasureProgress` events (spec §14.9.1 §§13 GF4
    /// two-phase commit). Single-region backends emit one event; multi-
