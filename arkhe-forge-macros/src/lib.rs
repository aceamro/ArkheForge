...
//! `BAND` / `IDEMPOTENT`).
//!
//! One attribute — `#[arkhe_pure]` (v0.12 도입) — asserts that an
//! `Action::compute` body conforms to E14.L1 R4-J Subset-Rust purity
//! (clock / RNG / I/O / FFI deny-list). Backed by
...
/// style function body conforms to E14.L1 R4-J Subset-Rust purity. Backed
/// by `arkhe-subset-rust-check`; emits a `compile_error!` per violation
/// site. Spec anchor: E14.L1 (v0.12 도입, MC).
///
/// Currently uses the v0.12 first-cut policy (clock + RNG deny-list).
/// Cryptographer review may extend to I/O / FFI deny-list in subsequent
/// rounds; the policy expansion is non-breaking (additive).
///
/// ## Usage
///
/// ```ignore
...
pub fn arkhe_pure(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let violations = arkhe_subset_rust_check::check_purity_v0_12(&item_fn);
    ...
    let errors: TokenStream2 = violations
        ...
        .map(|v| {
            let msg = format!(
                "E14.L1 R4-J Subset-Rust purity violation: \
                 `{}` ({}). \
